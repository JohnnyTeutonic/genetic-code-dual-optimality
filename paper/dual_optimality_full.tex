...
\begin{abstract}
The standard genetic code is ``optimal'' in two separate literatures: it is
highly error-resistant, and it is decoded by a remarkably small set of tRNAs. We
prove these are the same optimum: the wobble-fibre constancy that uniquely
...
tRNAs needed to decode the code under superwobble. The minimiser of the decoding
number is exactly the maximiser of silent edges. Because this coupling is
specific to superwobble (the four-way decoding by which organelles and reduced
genomes use minimal tRNA sets), it predicts that decoding redundancy above the
minimum grows with genome size---confirmed across 390 genomes
($p\approx6\times10^{-10}$, phylogenetically controlled). If one structural
property yields both error-resistance and decoding economy, neither need be the
other's selective cause, and the adaptive-versus-neutral debate is, for this
component, ill-posed.
\end{abstract}
...
The author used Claude Opus 4.6 (Anthropic) for manuscript preparation and
literature review.
